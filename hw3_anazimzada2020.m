...
e_quant_uniform = entropy(uint8(G_quant_uniform));
...
snr_uniform = snr(G, G - G_dequant_uniform);
disp(snr_uniform);


% c
G_quant_semiuniform = G_quant_uniform;
e_quant_semiuniform = entropy(uint8(G_quant_semiuniform));
...
snr_semiuniform = snr(G, G - G_dequant_semiuniform);
...
e_quant_nonuniform = entropy(uint8(G_quant_nonuniform));
disp(e_quant_nonuniform);

G_dequant_nonuniform = G_quant_nonuniform;
for i = 1:row
    for j = 1:col
        f = G_quant_nonuniform(i, j);
        if f == 0
...
snr_nonuniform = snr(G, G - G_dequant_nonuniform);
disp(snr_nonuniform);
...
r0X_clustering = X(0 * d + 1);
r1X_clustering = X(1 * d + 1);
r2X_clustering = X(2 * d + 1);
r3X_clustering = X(3 * d + 1);
r4X_clustering = X(4 * d + 1);
r5X_clustering = X(5 * d + 1);
r6X_clustering = X(6 * d + 1);
r7X_clustering = X(7 * d + 1);

clusteringX_iteration = 1;
while 1
    X_quant_clustering = X;
    for i = 1:N
        f = X(i);
        temp = 100000;
        if abs(f - r7X_clustering) < temp
            temp = abs(f - r7X_clustering);
        X_quant_clustering(i) = 7;
        end
        if abs(f - r6X_clustering) < temp
            temp = abs(f - r6X_clustering);
            X_quant_clustering(i) = 6;
        end
        if abs(f - r5X_clustering) < temp
            temp = abs(f - r5X_clustering);
            X_quant_clustering(i) = 5;
        end
        if abs(f - r4X_clustering) < temp
            temp = abs(f - r4X_clustering);
            X_quant_clustering(i) = 4;
        end
        if abs(f - r3X_clustering) < temp
            temp = abs(f - r3X_clustering);
            X_quant_clustering(i) = 3;
        end
        if abs(f - r2X_clustering) < temp
            temp = abs(f - r2X_clustering);
            X_quant_clustering(i) = 2;
        end
        if abs(f - r1X_clustering) < temp
            temp = abs(f - r1X_clustering);
            X_quant_clustering(i) = 1;
        end
        if abs(f - r0X_clustering) < temp
            temp = abs(f - r0X_clustering);
            X_quant_clustering(i) = 0;
        end
    end
    ...
    r0X_old = r0X_clustering;
    r1X_old = r1X_clustering;
    r2X_old = r2X_clustering;
    r3X_old = r3X_clustering;
    r4X_old = r4X_clustering;
    r5X_old = r5X_clustering;
    r6X_old = r6X_clustering;
    r7X_old = r7X_clustering;
    
    r0X_clustering = r0_sum / r0_count;
    r1X_clustering = r1_sum / r1_count;
    r2X_clustering = r2_sum / r2_count;
    r3X_clustering = r3_sum / r3_count;
    r4X_clustering = r4_sum / r4_count;
    r5X_clustering = r5_sum / r5_count;
    r6X_clustering = r6_sum / r6_count;
    r7X_clustering = r7_sum / r7_count;

    r0_flag = r0X_old == r0X_clustering;
    r1_flag = r1X_old == r1X_clustering;
    r2_flag = r2X_old == r2X_clustering;
    r3_flag = r3X_old == r3X_clustering;
    r4_flag = r4X_old == r4X_clustering;
    r5_flag = r5X_old == r5X_clustering;
    r6_flag = r6X_old == r6X_clustering;
    r7_flag = r7X_old == r7X_clustering;
    
    if r0_flag && r1_flag && r2_flag && r3_flag && r4_flag && r5_flag && r6_flag && r7_flag
        break
    else
        clusteringX_iteration = clusteringX_iteration + 1;
    end
end

disp(clusteringX_iteration);

e_Xquant_clustering = entropy(uint8(X_quant_clustering));
disp(e_Xquant_clustering);

X_dequant_clustering = X_quant_clustering;
for i = 1:N
    f = X_quant_clustering(i);
    if f == 0
        X_dequant_clustering(i) = r0X_clustering;
    elseif f == 1
        X_dequant_clustering(i) = r1X_clustering;
    elseif f == 2
        X_dequant_clustering(i) = r2X_clustering;
    elseif f == 3
        X_dequant_clustering(i) = r3X_clustering;
    elseif f == 4
        X_dequant_clustering(i) = r4X_clustering;
    elseif f == 5
        X_dequant_clustering(i) = r5X_clustering;
    elseif f == 6
        X_dequant_clustering(i) = r6X_clustering;
    else
        X_dequant_clustering(i) = r7X_clustering;
    end
end

mse_Xclustering = immse(X, X_dequant_clustering);
disp(mse_Xclustering);
...
r0_clustering = G_unique(1 * d);
r1_clustering = G_unique(2 * d);
r2_clustering = G_unique(3 * d);
r3_clustering = G_unique(4 * d);
r4_clustering = G_unique(5 * d);
r5_clustering = G_unique(6 * d);
r6_clustering = G_unique(7 * d);
r7_clustering = G_unique(8 * d);
...
e_quant_clustering = entropy(uint8(G_quant_clustering));
...
snr_clustering = snr(G, G - G_dequant_clustering);
disp(snr_clustering);
...
snr_G10 = snr(G, G - G10_dequantized);
disp(snr_G10);
