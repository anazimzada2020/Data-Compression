...
plot(x, x_fft_hat, 'o');
xlabel("Actual values of 'x'");
ylabel("Values after Inverse Fourier transform");
title("x VS x̂ (FFT)");

plot(y, y_fft_hat, 'ro');
xlabel("Actual values of 'y'");
ylabel("Values after Inverse Fourier transform");
title("y VS ŷ (FFT)");
...
x_fft_snr = snr(x, x_fft_hat);
y_fft_snr = snr(y, y_fft_hat);
...
plot(x, x_dct_hat, 'o');
xlabel("Actual values of 'x'");
ylabel("Values after Inverse Discrete Cosine transform");
title("x VS x̂ (DCT)");
...
x_dct_snr = snr(x, x_dct_hat);
y_dct_snr = snr(y, y_dct_hat);
...
hold on;
plot(x, x_dct_hat, 'b--');
plot(x, x_hadamard_hat, 'm:');
hold off;
title("x VS x̂");
xlabel("Actual values of 'x'");
ylabel("Values after Inverse Transformations");
legend('Fourier transform','Discrete Cosine transform', 'Hadamard transform');

plot(y, y_fft_hat, 'go');
hold on;
plot(y, y_dct_hat, 'bo');
plot(y, y_hadamard_hat, 'mo');
hold off;
title("y VS ŷ");
xlabel("Actual values of 'y'");
ylabel("Values after Inverse Transformations");
...
for i = 1:15
    x_dct_snr_list = [x_dct_snr_list; snr(x, x_dct_hat_list(i, :))];
end

scatter(1:15, x_dct_snr_list, 'p');
axis([0 20 0 20]);
xlabel("'n'");
ylabel("Signal-to-noise ratio");
title("'n' VS SNR");

disp("Except n=15, it is observed that SNR is being increased in a small amounts.");
